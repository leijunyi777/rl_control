...
from model_ode import Main7GapFollowingDynamics, get_veh12_gap, make_car_from_state
...
EXPORT_PATH = "main12_nomove_random_ego_x.gif"
EXPORT_FPS = 20
...
class Main11NoMoveDynamics(Main7GapFollowingDynamics):
    """Main11 no-control dynamics with the latest RBF confidence u(t)."""
...
def snapshot(t, state, diag, veh12_gap, collided):
    return {
        ...
        "b_t": float(diag["b_t"]),
        "u_t": float(diag["u_t"]),
        "d_gap": float(diag["d_gap"]),
        "dv_gap": float(diag["dv_gap"]),
...
def draw_scene(ax_anim, ax_z, ax_gap, frame, frames, lane_width, sim_time, collision_radius, desired_gap, safe_gap, vehicle_l):
    ...
    conf_hist = [item["confidence"] for item in frames]
    d_gap_hist = [item["d_gap"] for item in frames]
    ...
    title = f"Time: {frame['time']:.2f}s | Main12 No-Control RBF-u Comparison | ego x0 random +/-5m"
    if frame["collided"]:
        title += " | COLLISION"
    ...
    ax_gap.plot(times, conf_hist, "brown", linestyle=":", linewidth=2, label="Confidence")
    ax_gap.plot(times, d_gap_hist, "tab:blue", linestyle="--", linewidth=1.5, label="d_gap")
    ...
    ax_gap.set_title("Latest Gap Signals for b(t) and RBF u(t)")
...
def export_animation(frames, lane_width, sim_time, collision_radius, desired_gap, safe_gap, vehicle_l, export_path):
    ...
    def draw_frame(index):
        draw_scene(ax_anim, ax_z, ax_gap, frames[index], frames[: index + 1], lane_width, sim_time, collision_radius, desired_gap, safe_gap, vehicle_l)
...
def main():
    ...
    vehicle_l = 2.8
    ego_x0 = sample_ego_x()
    print(f"Main12 nomove ego initial x = {ego_x0:.3f} m")
    ...
    veh3 = EgoVehicleOdeModel(id="Veh 3 (Ego Constant Speed)", x=ego_x0, y=lane_width * 0.5, v=15.0, L=vehicle_l, color="lightgreen")

    dynamics = Main11NoMoveDynamics(veh1, veh2, veh3, desired_gap=DESIRED_GAP, gap_safe=GAP_SAFE)
    ...
    for i in range(int(SIM_TIME / DT)):
        ...
        if RENDER_DURING_SIM and (i % 4 == 0 or collided):
            draw_scene(ax_anim, ax_z, ax_gap, frames[-1], frames, lane_width, SIM_TIME, collision_radius, dynamics.desired_gap, dynamics.gap_safe, vehicle_l)
            plt.pause(0.01)
