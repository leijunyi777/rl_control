...
EXPORT_PATH = "main11_move_confidence_u.gif"
EXPORT_FPS = 20
...
class Main11MoveDynamics(Main7GapFollowingDynamics):
    """Main11 move dynamics with the latest RBF confidence u(t)."""
...
def draw_scene(ax_anim, ax_z, ax_dist, frame, frames, lane_width, sim_time, collision_radius, desired_gap, safe_gap, vehicle_l):
    ...
    ego = make_car_from_state("Veh 3 (Ego Main11 Control)", frame["ego_state"], "lightgreen", vehicle_l)
    ...
    title = f"Time: {frame['time']:.2f}s | Main11 RBF-u Control"
    if frame["collided"]:
        title += " | COLLISION"
    ax_anim.set_title(title)

    ax_z.cla()
    ax_z.plot(times, old_z_hist, "b-", linewidth=1.8, label="Old Formula z")
    ax_z.plot(times, new_z_hist, "m-", linewidth=2.4, label="New Formula z")
    ax_z.plot(times, b_hist, "g--", linewidth=1.8, label="b(t)")
    ax_z.plot(times, u_hist, "purple", linestyle="--", linewidth=1.8, label="u(t)")
    ax_z.plot(times, conf_hist, "orange", linestyle=":", linewidth=2, label="Confidence")
    ...
    ax_z.set_title("Latest Opinion Dynamics: RBF u(t)")
...
def main():
    ...
    veh3 = EgoVehicleOdeModel(id="Veh 3 (Ego Main11 Control)", x=20.0, y=lane_width * 0.5, v=15.0, L=vehicle_l, color="lightgreen")

    dynamics = Main11MoveDynamics(veh1, veh2, veh3, desired_gap=DESIRED_GAP, gap_safe=GAP_SAFE)
