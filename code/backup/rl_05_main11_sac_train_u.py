...
_MAIN11_MOVE_PATH = Path(__file__).with_name("sim_09_main11_rbf_u_move.py")
_MAIN11_SPEC = importlib.util.spec_from_file_location("main11_move_module", _MAIN11_MOVE_PATH)
_MAIN11_MOVE = importlib.util.module_from_spec(_MAIN11_SPEC)
_MAIN11_SPEC.loader.exec_module(_MAIN11_MOVE)

Main11MoveDynamics = _MAIN11_MOVE.Main11MoveDynamics
DESIRED_GAP = _MAIN11_MOVE.DESIRED_GAP
GAP_SAFE = _MAIN11_MOVE.GAP_SAFE
Z_DAMPING = _MAIN11_MOVE.Z_DAMPING
Z_ALPHA = _MAIN11_MOVE.Z_ALPHA
...
U_LOW = 0.0
U_HIGH = 3.0
POLICY_PATH = "main11_sac_u_policy.pth"
RESULT_FIG_PATH = "main11_sac_u_training_result.png"
...
class Main11SacUDynamics(Main11MoveDynamics):
    """Main11 dynamics where SAC directly supplies u(t)."""
...
class Main11SacUEnv:
    """SAC environment based on main11-move, with action = u(t)."""
    ...
    def render(self, info):
        ...
        title = f"Time: {self.t:.2f}s | Main11 SAC u(t) training"
        if info["collided"]:
            title += " | COLLISION"
        if info["success"]:
            title += " | SUCCESS"
...
def plot_training_results(episode_rewards, episode_progress, episode_collisions, episode_successes, episode_mean_u):
    ...
    axes[0].set_title("Main11 SAC u(t) Training Result")
    axes[0].legend()
    axes[0].grid(True)
...
def train():
    ...
    env = Main11SacUEnv(render=RENDER_DURING_TRAINING)
    ...
    for episode in range(1, NUM_EPISODES + 1):
        ...
        print(
            ...
            f"mean_u={episode_mean_u[-1]:.3f} | "
            f"success={last_info['success']} | collision={last_info['collided']} | steps={total_steps}"
        )

    plot_training_results(episode_rewards, episode_progress, episode_collisions, episode_successes, episode_mean_u)
    torch.save(
        {
