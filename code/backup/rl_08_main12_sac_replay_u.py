...
EXPORT_PATH = "main12_sac_u_replay.gif"
EXPORT_FPS = 20
...
def collect_replay_frames(agent, env):
    state = env.reset()
    print(f"Main12 replay ego initial x = {env.ego_x0:.3f} m")
    frames = [snapshot(env)]
    total_reward = 0.0
...
def export_replay_animation(frames, env, export_path):
    ...
    def draw_frame(index):
        ...
        ego = make_car_from_state("Veh 3 (Ego Main12 SAC-u)", frame["ego_state"], "lightgreen", env.vehicle_l)
        ...
        title = f"Time: {frame['time']:.2f}s | Main12 SAC u(t) replay | ego x0={frame['ego_x0']:.2f}m"
        if frame["success"]:
            title += " | SUCCESS"
        if frame["collided"]:
            title += " | COLLISION"
        ...
        ax_z.set_title("Policy Attention and Main12 Opinion Dynamics")
        ...
        ax_dist.axhline(env.gap_safe, color="orange", linestyle="--", linewidth=1.8, label="Safe Gap")
...
def run_policy_once():
    ...
    env = Main12SacUEnv(render=False)
