...
from rl_05_main11_sac_train_u import (
    POLICY_PATH,
    SACAgent,
    SEED,
    Main11SacUEnv,
...
from model_ode import KinematicBicycleModel
from model_ode import draw_car, draw_environment


EXPORT_ANIMATION = True
EXPORT_PATH = "main11_sac_u_replay.gif"
EXPORT_FPS = 20
...
def load_trained_agent(policy_path, device):
    if not os.path.exists(policy_path):
        raise FileNotFoundError(
            f"Cannot find {policy_path}. Run main11_sac_train.py first to generate the trained policy."
        )
...
def snapshot(env, info=None, reward=0.0, total_reward=0.0):
    ...
    return {
        ...
        "u_t": float(data["u_t"]),
        "formula_u_t": float(data["formula_u_t"]),
        "b_t": float(data["b_t"]),
        "d_gap": float(data["d_gap"]),
        "dv_gap": float(data["dv_gap"]),
...
def collect_replay_frames(agent, env):
    ...
    frames = [snapshot(env)]
    total_reward = 0.0
    last_info = {"success": False, "collided": False, "lane_progress": 0.0, "time": 0.0, "u_t": 0.0}
    ...
    for step_index in range(int(env.sim_time / env.dt)):
        ...
        if PRINT_EVERY_STEP:
            print(
                ...
                f"u(t)={info['u_t']:.3f} | formula_u={info['formula_u_t']:.3f} | "
                f"b(t)={info['b_t']:+.3f} | z={info['z_new']:+.3f}"
            )
...
def export_replay_animation(frames, env, export_path):
    ...
    u_hist = np.array([frame["u_t"] for frame in frames])
    formula_u_hist = np.array([frame["formula_u_t"] for frame in frames])
    ...
    def draw_frame(index):
        ...
        ego = make_car_from_state("Veh 3 (Ego Main11 SAC-u)", frame["ego_state"], "lightgreen", env.vehicle_l)
        ...
        title = f"Time: {frame['time']:.2f}s | Main11 SAC u(t) policy replay"
        if frame["success"]:
            title += " | SUCCESS"
        if frame["collided"]:
            title += " | COLLISION"
        ...
        ax_z.plot(times[: index + 1], u_hist[: index + 1], "purple", linewidth=1.8, label="Policy u(t)")
        ax_z.plot(times[: index + 1], formula_u_hist[: index + 1], "orange", linestyle="--", linewidth=1.8, label="RBF Formula u(t)")
        ax_z.plot(times[: index + 1], bt_hist[: index + 1], "g--", linewidth=1.8, label="b(t)")
        ...
        ax_z.set_title("Policy Attention and Main11 Opinion Dynamics")
...
def run_policy_once():
    ...
    env = Main11SacUEnv(render=False)
    ...
    if EXPORT_ANIMATION:
        ...
        plt.plot([frame["time"] for frame in frames], [frame["u_t"] for frame in frames], label="Policy u(t)")
        plt.xlabel("Time (s)")
        plt.legend()
        plt.grid(True)
        plt.show()
