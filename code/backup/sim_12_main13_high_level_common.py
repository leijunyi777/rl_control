    ...
    def _load(self):
        ...
        if not os.path.exists(self.policy_path):
            raise FileNotFoundError(
                f"Cannot find {self.policy_path}. Set USE_RL_U=False or train rl_07_main12_sac_train_u_random_ego.py first."
            )
        import torch

        spec = importlib.util.spec_from_file_location(
            "main12_sac_train_module",
            Path(__file__).with_name("rl_07_main12_sac_train_u_random_ego.py"),
        )
