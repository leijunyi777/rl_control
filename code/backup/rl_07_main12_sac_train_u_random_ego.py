...
_MAIN11_MOVE_PATH = Path(__file__).with_name("sim_11_main12_random_ego_move.py")
...
class Main12SacUEnv:
    """Main12 SAC-u environment with randomized ego initial x position."""
    ...
    def render(self, info):
        ...
        title = f"Time: {self.t:.2f}s | Main12 SAC u(t) training | ego x0={self.ego_x0:.2f}m"
        if info["collided"]:
            title += " | COLLISION"
        if info["success"]:
            title += " | SUCCESS"
