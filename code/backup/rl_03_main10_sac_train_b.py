...
from model_ode import get_veh12_gap
from model_ode import (
    EgoVehicleOdeModel,
...
from model_ode import draw_car, draw_environment


_MAIN10_MOVE_PATH = Path(__file__).with_name("sim_07_main10_gap_z_move.py")
_MAIN10_SPEC = importlib.util.spec_from_file_location("main10_move_module", _MAIN10_MOVE_PATH)
_MAIN10_MOVE = importlib.util.module_from_spec(_MAIN10_SPEC)
_MAIN10_SPEC.loader.exec_module(_MAIN10_MOVE)

Main10MoveDynamics = _MAIN10_MOVE.Main10MoveDynamics
DESIRED_GAP = _MAIN10_MOVE.DESIRED_GAP
GAP_SAFE = _MAIN10_MOVE.GAP_SAFE
Z_DAMPING = _MAIN10_MOVE.Z_DAMPING
Z_ALPHA = _MAIN10_MOVE.Z_ALPHA
...
BT_LOW = -1.0
BT_HIGH = 1.0
POLICY_PATH = "main10_sac_policy.pth"
RESULT_FIG_PATH = "main10_sac_training_result.png"
...
class Main10RlDynamics(Main10MoveDynamics):
    """Main10 dynamics where SAC directly supplies b(t)."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.action_b_t = 0.0

    def set_action_b_t(self, b_t):
        self.action_b_t = float(np.clip(b_t, BT_LOW, BT_HIGH))
    ...
    def diagnostics(self, state):
        ...
        gap_b_t = diag["b_t"]
        diag["gap_formula_b_t"] = gap_b_t
        diag["b_t"] = self.action_b_t
...
class Main10SacMergeEnv:
    """SAC environment based on main10-move, with action = b(t)."""
    ...
    def reset(self):
        ...
        self.dynamics = Main10RlDynamics(
            self.veh1,
            self.veh2,
            self.ego,
        ...
        self.t_hist, self.z_hist, self.bt_hist, self.gap_bt_hist, self.ut_hist = [], [], [], [], []
        self.dist1_hist, self.dist2_hist, self.veh12_gap_hist = [], [], []
        return self._get_obs()

    def _action_to_bt(self, action):
        action = np.clip(np.asarray(action, dtype=np.float32), -1.0, 1.0)
        return float(BT_LOW + 0.5 * (action[0] + 1.0) * (BT_HIGH - BT_LOW))
    ...
    def step(self, action):
        action = np.clip(np.asarray(action, dtype=np.float32), -1.0, 1.0)
        action_b_t = self._action_to_bt(action)
        self.dynamics.set_action_b_t(action_b_t)
        ...
        info = {
            "b_t": action_b_t,
            "gap_formula_b_t": diag["gap_formula_b_t"],
            "u_t": diag["u_t"],
            "z_new": self.state[17],
        ...
        self.bt_hist.append(action_b_t)
        self.gap_bt_hist.append(diag["gap_formula_b_t"])
        self.ut_hist.append(diag["u_t"])
    ...
    def render(self, info):
        ...
        title = f"Time: {self.t:.2f}s | Main10 SAC b(t) training"
        if info["collided"]:
            title += " | COLLISION"
        if info["success"]:
            title += " | SUCCESS"
        ...
        self.ax_z.plot(self.t_hist, self.bt_hist, "g--", linewidth=1.8, label="RL b(t)")
        self.ax_z.plot(self.t_hist, self.gap_bt_hist, "gray", linestyle="--", linewidth=1.5, label="Gap Formula b(t)")
        self.ax_z.plot(self.t_hist, self.ut_hist, "purple", linestyle=":", linewidth=1.8, label="u(t)")
        ...
        self.ax_z.set_title("RL Bias Injected into New Opinion Dynamics")
        ...
        self.ax_dist.axhline(self.gap_safe, color="orange", linestyle="--", linewidth=1.8, label="Safe Gap 15m")
...
def plot_training_results(episode_rewards, episode_progress, episode_collisions, episode_successes, episode_mean_bt):
    ...
    axes[0].set_title("Main10 SAC Training Result")
    axes[0].legend()
    axes[0].grid(True)
    ...
    axes[2].plot(episodes, episode_mean_bt, color="tab:orange", label="Mean RL b(t)")
    axes[2].set_ylabel("Mean b(t)")
    axes[2].set_ylim(BT_LOW - 0.1, BT_HIGH + 0.1)
    axes[2].legend()
    axes[2].grid(True)

    axes[3].plot(episodes, episode_collisions, color="tab:red", label="Collision")
    axes[3].set_ylabel("Collision")
    axes[3].set_ylim(-0.05, 1.05)
    axes[3].legend()
    axes[3].grid(True)

    axes[4].plot(episodes, episode_successes, color="tab:purple", label="Success")
...
def train():
    ...
    env = Main10SacMergeEnv(render=RENDER_DURING_TRAINING)
    ...
    for episode in range(1, NUM_EPISODES + 1):
        ...
        episode_reward = 0.0
        bt_values = []
        last_info = {"lane_progress": 0.0, "collided": False, "success": False, "b_t": 0.0}
        ...
        episode_mean_bt.append(float(np.mean(bt_values)) if bt_values else 0.0)

        print(
            f"Episode {episode:04d} | reward={episode_reward:8.2f} | "
            f"progress={last_info['lane_progress']:.3f} | "
            f"mean_b={episode_mean_bt[-1]:+.3f} | "
